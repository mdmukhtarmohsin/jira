...
export function useSprintPlanningData() {
  ...
  const fetchBacklogTasks = async (teamId: string) => {
    ...
    try {
      // Get tasks for the selected team that are not in any sprint
      const { data: tasksData, error: tasksError } = await supabase
        ...
        `)
        .eq("team_id", teamId)
        .eq("status", "todo") // Only get tasks that are in the backlog
    ...
    try {
      ...
      if (sprintData.tasks.length > 0) {
        ...
        if (validTaskIds.length > 0) {
          ...
          const { error: tasksError } = await supabase.from("sprint_tasks").insert(sprintTasks)

          if (tasksError) {
            console.error("Sprint tasks insertion error:", tasksError)
            throw new Error(`Failed to add tasks to sprint: ${tasksError.message}`)
          }
