    ...
    try {
      ...
      if (tasksForAI.length === 0) {
        toast({
          title: "Error",
          description:
            "No tasks available for planning. Please add tasks to your backlog or paste custom tasks.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (teamCapacity === 0) {
        toast({
          title: "Error",
          description:
            "No team capacity available. Please ensure team members are assigned.",
          variant: "destructive",
        });
        return;
      }
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Let AI suggest an optimal sprint plan based on your tasks and team
              capacity
            </CardDescription>
          </div>
        </div>
          ...
          <>
            <div className="space-y-2">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                You can paste your own tasks or use the tasks from your backlog.
                Each line should be a task. Optionally include priority
                [high/medium/low] and story points (5).
              </p>
              <Textarea
              ...
              <p className="text-xs text-gray-600 dark:text-gray-400">
                Example: "Implement login page [high] (5)"
              </p>
            </div>
            ...
            <div className="flex justify-between items-center">
              <div>
                <p className="text-sm font-medium">
                  Team capacity:{" "}
                  {teamMembers.reduce((sum, m) => sum + m.capacity, 0)} hours
                </p>
                <p className="text-sm font-medium">
                  Available tasks:{" "}
                  {customTasks
                    ? parseCustomTasks(customTasks).length
                    : tasks.length}
                </p>
              </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              <h3 className="font-medium text-blue-900">
                Tasks ({aiSuggestion.recommendedTasks.length})
              </h3>
              <p className="text-blue-700">
                {aiSuggestion.totalStoryPoints} story points
              </p>
                  ...
                  return (
                    <div key={index} className="text-sm">
                      <p className="font-medium">
                        ...
                        {assignment.storyPoints} points
                      </p>
                      <p className="text-gray-600 dark:text-gray-400">
                        {assignment.tasks.length} tasks assigned
                      </p>
                    </div>
                  );
                })}
